...
> 확정일: 2026-08-07 · 작성: B(백엔드) · **v1.1.4** (2026-08-16 필드 추가: `policy.first_seen`·`policy.is_new`)
...
## 변경 이력 (v1.1.3 → v1.1.4, 2026-08-16 — 필드 추가 2건)

| # | 변경 | 사유 |
| --- | --- | --- |
| 1 | **`policy.first_seen`(date·null) 추가** — 크롤러가 정책을 처음 관측한 날(저장값, A의 diff 산출). 없으면 `null` | NEW 배지(발표 ③)의 데이터 근거. 현재 정책 8건은 전부 `null` — 응답 변화는 first_seen 있는 정책이 머지된 뒤 나타난다 |
| 2 | **`policy.is_new`(bool) 추가** — `first_seen`이 `as_of` 기준 **7일 이내**면 `true`. 저장하지 않고 서버가 계산 | `as_of` 기반이라 시연 재현 가능(What-if로 과거를 보면 미래 관측 정책은 신규 아님). C는 이 값만 보고 NEW 배지를 켠다. 창 크기(7일)는 시연 관점에서 조정 가능 |

## 변경 이력 (v1.1.2 → v1.1.3, 2026-08-15 — 가구 단위 판정 반영, 필드 변경 없음)

| # | 변경 | 사유 |
| --- | --- | --- |
| 1 | **`for_member`가 실제로 채워진다** — 정책의 `beneficiary`가 가리키는 가구원(본인/배우자/자녀/부/모)을 기준으로 각각 판정. beneficiary 매핑·세대주 폴백 규칙을 "분류 규칙" 절에 명시 | 가구 판정 구현(8/15). 응답 구조는 v1.1 약속대로 불변 — 항목 수만 늘 수 있다 |
| 2 | What-if 반영 표의 `members`: ❌ → **⚠️ 조건부** (beneficiary가 배우자·자녀·세대주인 정책에서만 결과가 바뀐다) | 현재 정책 데이터는 전부 본인·가구 대상이라 화면 변화가 없다 — C가 "안 먹는다"로 오인할 지점을 실측 기준으로 갱신 |
...
| `members` | 가구원 목록 | `relation` 허용값: **본인 / 배우자 / 자녀 / 부 / 모 / 기타** (profile.schema.json 확정본 — 부·모는 조력자 모드용). **가구 판정(8/15 구현)이 이 목록을 정본으로 순회한다** — 정책의 `beneficiary`가 가리키는 가구원의 나이로 판정 (매핑 규칙은 "분류 규칙" 절) |
...
| `members` | ⚠️ 조건부 | **가구 판정 구현됨(8/15).** 정책의 `beneficiary`가 배우자·자녀·세대주일 때만 결과가 바뀐다 — **현재 정책 데이터는 전부 본인·가구 대상이라 아직 화면 변화가 없다** (자녀 대상 정책 머지 시 동작) |
| `region` | ❌ | 판정 조건이 아니다(표시용) |
| `lifecycle` | ❌ | 생애주기는 분류 축이지 판정 조건이 아니다 |

**C 주의**: 위 What-if 예시(`members`에 자녀 추가 — "둘째가 태어나면?")는 **엔진에는 반영됐지만
(8/15 가구 판정), 자녀·배우자·세대주 대상 정책이 데이터에 없으면 화면이 변하지 않는다.** 버그가
아니다. 현재 데이터 기준으로 결과가 달라지는 노브는 여전히 `birth_date`(나이)·`move_in_date`
(전입일)·최상위 `as_of`(시간 이동) 세 개다. 검증된 시연 조합은 `docs/demo-scenarios.md` 참조.
...
가구 판정(2026-08-15 구현)에서는 같은 정책이 본인 기준 `eligible`, 배우자 기준 `docs_needed`에
동시에 나타날 수 있다 — 이것이 정상이므로 **C는 정책 ID 기준 중복 제거를 하지 않는다.**

### 가구 판정 — beneficiary → 판정 기준 가구원 매핑 (2026-08-15 구현)

정책의 `beneficiary`가 가리키는 가구원의 **나이**로 판정하고, 그 가구원을 `for_member`에 적는다.
거주개월·가구 유형은 가구 공통값(프로필의 `move_in_date`·`household_type`)으로 판정한다.

| `beneficiary` | 판정 기준 | `for_member` |
| --- | --- | --- |
| 본인 · 가구 | 프로필 본인 (가구는 대표자 기준 — 종전 동작 그대로) | `"본인"` |
| 배우자 | `relation: "배우자"`인 가구원 | `"배우자"` |
| 자녀 | `relation: "자녀"`인 가구원 **각각** — 자녀가 둘이면 항목도 둘 | `"자녀"` (항목별) |
| 세대주 | `is_householder: true`인 가구원. **미지정 폴백**: 본인이 성인(만 19세 이상)이면 본인, 미성년(조력자 모드)이면 부/모 중 첫 항목 | 해당 가구원의 relation |

- **기준 가구원이 가구에 없으면 그 정책은 비대상** (응답 제외) — 자녀 대상 정책 + 자녀 없는 가구,
  미성년 본인 + 부/모 미입력 등
- 자녀가 여럿이면 `for_member`가 똑같이 `"자녀"`인 항목이 나란히 온다 — 중복이 아니라 각 자녀의
  판정이다. 구분 라벨이 필요해지면 필드 **추가**로 해결한다 (구조 변경 없음)
...
- **`for_member`** — 판정 기준 가구원 (2026-08-15 가구 판정 구현으로 실제 채워짐). 값은 `relation` 허용값과 같다: `"본인"` / `"배우자"` / `"자녀"` / `"부"` / `"모"`. 매핑 규칙은 위 "분류 규칙" 절 — 응답 구조는 v1.1 약속대로 불변, 항목 수만 늘 수 있다
...
- **`policy.first_seen` / `policy.is_new`** — (v1.1.4 추가) 크롤러 최초 관측일(없으면 `null`) / 그 날이 `as_of` 기준 7일 이내인지(**저장값 아님 — 서버 계산**). **NEW 배지는 `is_new`만 보면 된다.** 첫 크롤 배치는 관측일이 전부 최근이라 배지가 일괄로 붙을 수 있다 — 창 조정은 D·C 판단
- **예정 추가 필드** — 모르는 필드는 무시하면 되므로 C 코드는 안 깨진다:
  - ~~`policy.is_new` 또는 `policy.first_seen`~~ → **추가됨 (v1.1.4, 8/16)** — 위 필드별 약속 참조
