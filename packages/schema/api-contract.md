...
- **`ai_summary`** — 각 판정 항목의 안내 문장. **nullable — `null`이면 C는 "요약 준비 중"을 표시한다.**
  (**확정 · 2026-08-12 구현 완료.** 재조회·폴링은 **불필요**해졌다 — 아래 참조)

  요약은 **(정책, 판정 결과) 조합마다 미리 만들어 `data/summaries.json`에 커밋**해 두고 API는
  읽기만 한다. 요청 경로에 LLM이 없어 응답이 지연되지 않고, 같은 조합에 재호출도 없다.
  남은 기간·날짜는 문구에 넣지 않으므로(화면이 따로 표시) `as_of`가 달라져도 문구는 같다.

  **`null`이 되는 경우 2가지** — C는 둘을 구분할 필요 없이 "요약 준비 중"을 표시하면 된다:
  1. 아직 그 조합의 요약을 만들지 않음 (새로 추가된 정책 등)
  2. **정책이 수정된 뒤 요약이 재생성되지 않음** — 서버가 정책 내용 해시를 대조해 낡은 문구를
     내보내지 않는다. 낡은 요약으로 틀린 금액·조건을 안내하느니 비워 두는 편이 안전하다
...
| ~~`ai_summary` 전달 방식 (nullable 재조회 vs 폴링)~~ | ~~8/13~~ | **해소(8/12)** — 배치 사전 생성으로 확정. 요약이 항상 준비돼 있어 재조회·폴링 자체가 불필요하다. 상세는 위 `ai_summary` 절 |
