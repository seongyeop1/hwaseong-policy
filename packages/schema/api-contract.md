...
| POST | `/evaluate` + `overrides` | What-if 시뮬레이터 (가상 프로필 병합) | 8/14 이후 (스펙은 8/13 확정) |
...
### What-if 요청 — `overrides` (초안, 8/13 확정 예정)
...
| 항목 | 기한 | 현재 상태 |
| --- | --- | --- |
| `overrides` 최종 스펙 | **8/13** | 본 문서의 초안(통째 교체 병합)대로 확정 예정. 이의 있으면 8/13 전에 제기 |
