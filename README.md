...
LLM은 요청 경로에 없다 → 판정은 순수 함수이고, 같은 입력이면 언제나 같은 결과가 나온다.
...
| 항목 | 링크 |
|---|---|
| **웹앱 (상시 접속)** | https://hwaseong-policy-five.vercel.app |
| 판정 API 상태 | https://hwaseong-policy-api.onrender.com/health |

첫 화면에서 생애주기를 고르고 프로필(생년월일·전입일·가구 유형)을 입력하면
**자격 충족 / 서류 확인 필요 / 예정(D-day)** 3분류로 결과가 나온다.
What-if 패널의 "6개월 뒤 · 자녀가 생기면"은 화면 연출이 아니라 서버 재판정이다.
...
| 8/18 밤 | 시연 영상·발표자료·README 완성 |
| **8/19 23:59** | **1차 제출 마감 (웹 제출).** 저녁까지 최종 점검 후 제출 |
