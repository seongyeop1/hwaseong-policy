...
- beneficiary: "세대주","본인","배우자","자녀","가구" 중 하나
  단, 원문에 수혜자가 명확히 특정되지 않으면 "본인"으로 두세요. URL·부서명·문맥으로 추론 금지.
- benefit: 지원 내용 한 줄 요약 (상세 페이지 표시용)
  반드시 금액·기간·횟수 등 핵심 수치를 포함하세요.
  예) "월 20만원 최대 12개월 지원", "연 100만원 한도 치료비 지원", "주 30시간 8주 일경험"
  수치가 없으면 "지원 내용 공고 확인 필요" 로 두세요. 제목을 되풀이하지 마세요.
...
- exclusions: 중복수급 제한이 명시된 경우만 배열로. 자격요건은 manual_conditions로.
- apply_channel: 신청 방법·경로 설명

## 추가 규칙
1. 자격요건 유형이 여러 개면(일반형/특화형, 1순위/2순위 등) 각각 구분해 manual_conditions에 별도 항목으로.
2. 원문에 없는 값을 추론해 채우지 마세요 — URL·부서명·beneficiary 등 모두 해당.
3. 나이를 출생연도·출생일 구간으로 명시한 경우 age.min/max 외에 manual_conditions에도 원문 그대로 적으세요.
4. "신청 제외대상" 목록을 빠짐없이 옮기되 자격요건은 manual_conditions로, 중복수급 제한만 exclusions로.
